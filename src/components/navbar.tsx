    ...
    <div className={`w-full py-6 ${styles.flexBetween} navbar relative bg-transparent z-[100]`}>
      ...
      <ul className="list-none sm:flex hidden justify-end items-center flex-1">
        {navigationLinks.map((nav, idx) => (
          <li key={nav.id} className={`font-montserrat font-normal cursor-pointer text-[16px] hover:text-white transition-all duration-500 ${idx === navigationLinks.length - 1 ? 'mr-0' : 'mr-10'} ${active === nav.id ? 'text-white' : 'text-lightWhite'}`} onClick={() => activeHandler(nav.id)}>
      ...
      <div className={'sm:hidden flex flex-1 justify-end items-center'}>
        ...
        <div className={`${!toggleNav ? 'hidden' : 'flex'} p-6 absolute top-20 right-0 left-0 w-full sidebar bg-black-gradient z-20`}>
          <ul className="list-none flex justify-center items-center flex-1 flex-col xs:flex-row">
            {navigationLinks.map((nav, idx) => (
              <li key={nav.id} className={`${idx === navigationLinks.length - 1 ? 'xs:mr-0' : 'xs:mr-10'} ${active === nav.id ? 'text-white' : 'text-lightWhite'} font-montserrat font-normal cursor-pointer text-[16px] hover:text-white transition-all duration-500 my-3 xs:my-0`} onClick={() => activeHandler(nav.id)}>
                <a href={`#${nav.id}`}>{nav.title}</a>
              </li>
            ))}
          </ul>
